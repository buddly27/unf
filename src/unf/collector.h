...
class Collector {
  public:
    ...
    ///
    /// This method searches for an item of type `SubItem` within the
    /// collector. If found, it returns a weak pointer to the item.
    /// Otherwise, it returns `nullptr`.
    template <class SubItem>
    PXR_NS::TfWeakPtr<SubItem> Get()
    {
        static_assert(std::is_base_of<T, SubItem>::value);
        auto it = _items.find(typeid(SubItem).name());
        return it != _items.end()
                   ? PXR_NS::TfStatic_cast<PXR_NS::TfWeakPtr<SubItem>>(
                         it->second)
                   : nullptr;
    }
    ...
    ///
    /// This method creates a new item of type `SubItem`, adds it to the
    /// collector, and returns a reference pointer to the newly added item.
    template <class SubItem, class... Args>
    PXR_NS::TfRefPtr<T> Add(Args&&... args)
    {
        static_assert(std::is_base_of<T, SubItem>::value);
        auto item =
            PXR_NS::TfCreateRefPtr(new SubItem(std::forward<Args>(args)...));
        _Add(item);
        return item;
    }
